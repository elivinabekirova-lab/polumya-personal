...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!url || !serviceKey) {
      throw new Error("Не налаштовані серверні ключі Supabase");
    }
    ...
    if (currentError || !current.user) {
      throw new Error("Сесію адміністратора не підтверджено");
    }
    ...
    if (!isAdmin) {
      throw new Error("Недостатньо прав адміністратора");
    }
    ...
    if (!login) {
      throw new Error("Введи логін латинськими літерами");
    }

    if (!name) {
      throw new Error("Введи ім’я працівника");
    }
    ...
    if (role === "employee" && !staffId) {
      throw new Error("Не знайдено ID працівника");
    }
    ...
    if (profileError) {
      ...
      throw new Error(
        `Акаунт створено, але профіль не збережено: ${profileError.message}`,
      );
    }
